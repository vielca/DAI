...
class dai:
    ...
    def initGui(self):
        ...
        icon_path = ':/plugins/DAI/icon.png'
        self.add_action(
            icon_path,
            text=self.tr(u'DAI'),
            callback=self.run,
    ...
    def dai_func_calc(self):    
        ...
        menu_layer = '{} - Results'.format(datetime.datetime.now().strftime("%d-%m-%Y"))
        groups = []
        groups.append(root.findGroup(menu_layer))
        for group in groups:
        ...
        group_result_data = root.insertGroup(0, menu_layer)
        ...
        dst_crop_resample_ini = os.path.join(working_folder, "{}_original.tiff".format(t1.strftime('%Y%m%d')))
        ...
        dst_crop_resample_end = os.path.join(working_folder, "{}_original.tiff".format(t2.strftime('%Y%m%d')))
        options = gdal.WarpOptions(width = DIM_X, height = DIM_Y)
        ...
        self.cpr_load_map_raster_in_group(dst_crop_resample_ini, group_result_data)
        for i in interval:
            ...
            dst_ds_interpolation.SetProjection(crop_tiff_1.GetProjection())
            dst_ds_interpolation.SetGeoTransform(crop_tiff_1.GetGeoTransform())
            ...
            proj = osr.SpatialReference(wkt=crop_tiff_1.GetProjection())
            # print(proj.GetAttrValue('AUTHORITY',1))
            
            out_band.FlushCache()        
            # QMessageBox.information(self.iface.mainWindow(), "Advertisement", 'debug: '\
            #                         +  str(proj.GetAttrValue('AUTHORITY',1)))
            self.cpr_load_map_raster_in_group2(dst_results, group_result_data, proj.GetAttrValue('AUTHORITY',1))
            
            del dst_ds_interpolation    
        self.cpr_load_map_raster_in_group(dst_crop_resample_end, group_result_data)
        QMessageBox.information(self.iface.mainWindow(), "Advertisement", 'All layers have been developed correctly')
        self.dlg.close()
    ...
    def cpr_load_map_raster_in_group2(self,pathRasterMap, group, crs_tag):
        if self.dai_exists(pathRasterMap):
            return True
        returnIsValidRasterMap = False
        pathRasterMap = pathRasterMap.strip()    
        ...
        if (os.path.exists (pathRasterMap)):    
            ...
            baseNameRasterMap = os.path.splitext(baseNameRasterMap)[0]
            layerRasterMap = QgsRasterLayer(pathRasterMap, baseNameRasterMap)
            layerCrs = QgsCoordinateReferenceSystem.fromEpsgId(int(crs_tag))
            layerRasterMap.setCrs(layerCrs)
